...
let Settings = (env) ? require(Path.resolve('config/testing.settings.json')) : require(Path.resolve('config/settings.json'));
...
exports.update = {
    handler: (request, reply) => {
        ...
        .then(() => {
            ...
            Settings = Object.assign(Settings, request.payload);

            if (env) {
                FS.writeFileSync(Path.resolve('config/testing.settings.json'), JSON.stringify(Settings, null, 4));
            }
            else {
